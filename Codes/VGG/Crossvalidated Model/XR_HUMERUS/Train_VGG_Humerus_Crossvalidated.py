...
t1=time.time()
X = pickle.load(open("Uniform_X_HUMERUS_HIST_256.pickle","rb"))
Y = pickle.load(open("Uniform_Y_HUMERUS_HIST_256.pickle","rb"))
t2=time.time()
...
model_checkpoint = ModelCheckpoint('Humerus_VGG_Model_by_Machine.h5', verbose=1, save_best_only=True)
...
def fit_and_evaluate(X_Train, X_Test, Y_Train, Y_Test, Fold_No, y_test_fold_ROC):
    ...
    model.save('Humerus_VGG19_Model_' + str(Fold_No) + '.h5')
