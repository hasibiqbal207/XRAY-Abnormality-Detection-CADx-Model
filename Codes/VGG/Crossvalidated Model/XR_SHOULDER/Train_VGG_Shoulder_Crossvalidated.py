# -*- coding: utf-8 -*-
"""
Created on Sat Nov 10 12:34:53 2018
...
t1=time.time()
X = pickle.load(open("Uniform_X_SHOULDER_HIST_256.pickle","rb"))
Y = pickle.load(open("Uniform_Y_SHOULDER_HIST_256.pickle","rb"))
t2=time.time()
...
model_checkpoint = ModelCheckpoint('Shoulder_VGG_Model_by_Machine.h5', verbose=1, save_best_only=True)
...
def fit_and_evaluate(X_Train, X_Test, Y_Train, Y_Test, Fold_No, y_test_fold_ROC):
    ...
    model.save('Shoulder_VGG19_Model_' + str(Fold_No) + '.h5')
    ...
    # calculate AUC
    auc = roc_auc_score(y_test_fold_ROC, probs)
    auc = '%.3f' % auc
    print('AUC: ' +  auc)
    # calculate roc curve
    fpr, tpr, thresholds = roc_curve(y_test_fold_ROC, probs)
    ...
    pyplot.legend()
    pyplot.show()
    pyplot.savefig('ROC_' + str(Fold_No) + "_" + auc + '.png')
    ...
    file = open("Result_of_Model_" + str(Fold_No) + ".txt","w") 
...
Fold_No = 1
file = open("Time_To_Train_Model.txt","w") 
for train, test in kfold.split(X, Y):    
    ...
    train_index = train.shape
    train_index = train_index[0]
    
    test_index = test.shape
    ...
    for i in range(train_index):
        ...
        x_temp = X[j]
        y_temp = Y[j]
        x_train_fold.append(x_temp)
        y_train_fold.append(y_temp)
    
    for i in range(test_index):
        j = test[i]        
        x_temp = X[j]
        y_temp = Y[j]
        x_test_fold.append(x_temp)
        y_test_fold.append(y_temp)
    
    
    x_train_fold = array(x_train_fold)
    y_train_fold = array(y_train_fold)
    y_train_fold = to_categorical(y_train_fold,2)
    
    x_test_fold = array(x_test_fold)
    y_test_fold = array(y_test_fold)  
    y_test_fold_ROC = y_test_fold
    y_test_fold = to_categorical(y_test_fold,2)
    
    time.sleep(0.02)
    model_history.append(fit_and_evaluate(x_train_fold, x_test_fold, y_train_fold, y_test_fold, Fold_No, y_test_fold_ROC))
    ...
    file.write("\nTime Required To Train Model " + str(1) + " is : " + str(h) + " Hour " + str(m) + " Minute " + str(s) + " Second\n")
...
plt.plot(model_history[0].history['acc'], label='Training on Fold 1')
plt.plot(model_history[1].history['acc'], label='Training on Fold 2')
plt.plot(model_history[2].history['acc'], label='Training on Fold 3')
plt.plot(model_history[3].history['acc'], label='Training on Fold 4')
plt.plot(model_history[4].history['acc'], label='Training on Fold 5')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
plt.plot(model_history[0].history['val_acc'], label='Training on Fold 1')
plt.plot(model_history[1].history['val_acc'], label='Training on Fold 2')
plt.plot(model_history[2].history['val_acc'], label='Training on Fold 3')
plt.plot(model_history[3].history['val_acc'], label='Training on Fold 4')
plt.plot(model_history[4].history['val_acc'], label='Training on Fold 5')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
plt.plot(model_history[3].history['acc'], label='Train Accuracy Fold 4', color='blue', )
plt.plot(model_history[3].history['val_acc'], label='Val Accuracy Fold 4', color='blue', linestyle = "dashdot")
plt.plot(model_history[4].history['acc'], label='Train Accuracy Fold 5', color='darkorange', )
plt.plot(model_history[4].history['val_acc'], label='Val Accuracy Fold 5', color='darkorange', linestyle = "dashdot")
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
