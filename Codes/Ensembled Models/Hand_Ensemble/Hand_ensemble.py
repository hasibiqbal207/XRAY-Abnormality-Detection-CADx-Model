...
model_vgg = load_model('Hand_VGG_Model_by_Machine.h5')
model_vgg.compile(loss = "categorical_crossentropy", optimizer = optimizers.SGD(lr=0.0001, momentum=0.9), metrics=["accuracy"])

model_res = load_model('Hand_ResNET_Model_by_Machine.h5')
model_res.compile(loss = "categorical_crossentropy", optimizer = optimizers.SGD(lr=0.0001, momentum=0.9), metrics=["accuracy"])

model_dense = load_model('Hand_DenseNet_Model_by_Machine.h5')
model_dense.compile(loss = "categorical_crossentropy", optimizer = optimizers.Adam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=10, amsgrad=False),metrics=['accuracy'])    
...
for i in range(lenth):
    ...
    classifier3 = model_dense.predict(img)
    
    classifier_per_0 = (classifier1.item(0)*vgg_weight + classifier2.item(0)*res_weight + classifier3.item(0)*dense_weight)
    classifier_per_1 = (classifier1.item(1)*vgg_weight + classifier2.item(1)*res_weight + classifier3.item(1)*dense_weight)

#     Predicted Class Label Selection
    if(classifier_per_0 > 0.5):
        class_label = 0
    elif(classifier_per_1 > 0.5):
        class_label = 1
    else:
        drop += 1
        continue
