...
t1=time.time()
x_train = pickle.load(open("Splited_X_Train_Shoulder_HIST_256.pickle","rb"))
y_train = pickle.load(open("Splited_Y_Train_Shoulder_HIST_256.pickle","rb"))
y_train = to_categorical(y_train,2)
t2=time.time()
print("time to load :",(t2-t1))

t1=time.time()
x_test = pickle.load(open("Splited_X_Test_Shoulder_HIST_256.pickle","rb"))
y_test = pickle.load(open("Splited_Y_Test_Shoulder_HIST_256.pickle","rb"))
y_test_ROC = y_test
...
model_checkpoint = ModelCheckpoint('Shoulder_ResNET_Model_by_Machine.h5', verbose=1, save_best_only=True)
...
def final_model():
    ...
    # compile the model 
    model_final.compile(loss = "categorical_crossentropy", optimizer = optimizers.Adam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=10, amsgrad=False), metrics=["accuracy"])
    return model_final
...
def fit_and_evaluate(X_Train, X_Test, Y_Train, Y_Test, y_test_ROC):
    ...
    results = model.fit(X_Train, Y_Train, batch_size=4, epochs = epoch, callbacks=[early_stopping, model_checkpoint], verbose=1, validation_split=0.05)
    model.save('Shoulder_ResNET50_Model.h5')
